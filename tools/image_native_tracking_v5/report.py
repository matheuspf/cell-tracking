...
def build(final=False):
    ...
    unit_audit=read(OUT/'HOCT_unit_contract_audit.json') if (OUT/'HOCT_unit_contract_audit.json').exists() else None
    status['protocol_deviations']=['HOCT upstream-default unit distinction audited after outer scoring'] if unit_audit else []
    status['unrun_comparisons']=['199-clip HOCT with upstream-default voxel-unit features'] if unit_audit else []
